...
std::ostream &operator<<(std::ostream &ss, const quantize_type &qt) {
    switch (qt) {
        ...
        case quantize_type::per_tensor1: ss << "per_tensor2"; break;
...
std::string print_row(const sdpa_dims_t &p) {
    ...
    if (p.kdt != mdt::f16 || p.vdt != mdt::f16) { ss << "|" << p.qtype; }
    return ss.str();
}
...
void prim_sdpa_quant(const sdpa_dims_t &p, const sdpa_tensors_t &t,
    ...
    if ((key.get_desc().get_data_type() != mdt::f16
        ...
        auto key_dequantized_md = memory::desc(
                key.get_desc().get_dims(), mdt::f16, memory::format_tag::abcd);
        primitive_attr key_dequantized_attr;

        if (p.qtype == quantize_type::per_token_with_groups) {
            if (p.ksdt != mdt::undef) {
                key_dequantized_attr.set_scales(
                        DNNL_ARG_FROM, t.kq_mask, t.kq_groups, p.ksdt);
            }
            if (p.kzpdt != mdt::undef) {
                key_dequantized_attr.set_zero_points(
                        DNNL_ARG_SRC, t.kq_mask, t.kq_groups, p.kzpdt);
            }
        } else {
            if (p.ksdt != mdt::undef) {
                key_dequantized_attr.set_scales(
        ...
        key_dequantized = memory(key_dequantized_md, eng);
        auto key_dequantize_pd = dnnl::reorder::primitive_desc(
                key, key_dequantized, key_dequantized_attr, false);
        auto key_dequantize_prim = dnnl::reorder(key_dequantize_pd);
    ...
    if (value.get_desc().get_data_type() != mdt::f16
            ...
            && p.qtype != quantize_type::no_quantization) {
        auto value_dequantized_md = memory::desc(value.get_desc().get_dims(),
                mdt::f32, memory::format_tag::abcd);
        primitive_attr value_dequantized_attr;

        auto vs_mask = t.vs_mask;
        if (p.qtype == quantize_type::per_token_with_groups) {
            if (p.vsdt != mdt::undef) {
                value_dequantized_attr.set_scales(
                        DNNL_ARG_FROM, vs_mask, t.vs_groups, p.vsdt);
            }

            if (p.vzpdt != mdt::undef) {
                value_dequantized_attr.set_zero_points(
                        DNNL_ARG_SRC, vs_mask, t.vs_groups, p.vzpdt);
            }
        ...
        value_dequantized = memory(value_dequantized_md, eng);
        auto value_dequantize_pd = dnnl::reorder::primitive_desc(
                value, value_dequantized, value_dequantized_attr, false);
        auto value_dequantize_prim = dnnl::reorder(value_dequantize_pd);
...
GPU_TEST_P(sdpa_test_t, perf) {
    ...
    std::vector<microseconds> time_s8, time_f16;
    int iterations = 100;
    strm.wait();
    ...
    for (int i = 0; i < iterations; i++) {
        strm.wait();
        auto ss = steady_clock::now();
        loop_s8();
        strm.wait();
        long ln = duration_cast<microseconds>(steady_clock::now() - ss).count();
        min_s8 = std::min(ln, min_s8);
        //std::cout << "dt: (" << p << "): " << ln << "\n";
    }
    e = steady_clock::now();
    time_s8.push_back(duration_cast<microseconds>(e - s));

    std::cout << print_row(p) << "|" << min_s8 << "|" << std::endl;
    strm.wait();
}
